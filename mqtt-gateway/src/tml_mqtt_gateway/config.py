...
class KafkaConfig(BaseSettings):
    ...
    telemetry_topic: str = Field(
        default="tml.iot.telemetry", env="KAFKA_TELEMETRY_TOPIC"
    )
    ...
    dead_letter_topic: str = Field(
        default="tml.iot.dead_letter", env="KAFKA_DEAD_LETTER_TOPIC"
    )
...
class TMLGatewayConfig(BaseSettings):
    ...
    def validate_config(self) -> List[str]:
        ...
        # Database validation
        if not all(
            [self.database.host, self.database.username, self.database.password]
        ):
