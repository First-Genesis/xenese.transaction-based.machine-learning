...
class TMLKafkaProducer:
    ...
    def send_message(
        ...
        try:
            ...
            future = self.producer.send(
                topic=topic, value=enriched_message, key=key, headers=kafka_headers
            )
            ...
            self.logger.error(
                "Failed to send message to Kafka", topic_type=topic_type, error=str(e)
            )
    ...
    def _on_send_success(
        self, topic: str, message: Dict[str, Any], record_metadata
    ) -> None:
    ...
    def _on_send_error(self, topic: str, message: Dict[str, Any], exception) -> None:
        ...
        self.logger.error("Failed to send message", topic=topic, error=str(exception))
    ...
    def _send_to_dead_letter_queue(self, message: Dict[str, Any], error: str) -> None:
        ...
        try:
            ...
            future = self.producer.send(
                topic=self.config.dead_letter_topic, value=dead_letter_message
            )

            future.add_callback(
                lambda metadata: self.logger.info(
            ...
            self.logger.error(
                "Failed to send message to dead letter queue", error=str(e)
            )
    ...
    def process_mqtt_message(self, mqtt_topic: str, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.send_message(
                ...
                message=message,
                key=key,
                headers={"source_protocol": "mqtt", "mqtt_topic": mqtt_topic},
            )

        except Exception as e:
            self.logger.error(
                "Error processing MQTT message", mqtt_topic=mqtt_topic, error=str(e)
            )
            raise
