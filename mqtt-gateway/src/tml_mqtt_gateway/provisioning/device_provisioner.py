...
class DeviceProvisioner:
    ...
    async def create_provisioning_session(
        ...
        try:
            ...
            session_data = {
                ...
                + timedelta(minutes=self.provisioning_timeout_minutes),
                "attempts": 0,
    ...
    async def _generate_provisioning_payload(
        self, session_data: Dict[str, Any], method: ProvisioningMethod
    ) -> Dict[str, Any]:
        """Generate provisioning payload based on method"""
    ...
    async def provision_device(
        ...
        try:
            ...
            if (
                session_data
                and session_data["status"] == ProvisioningStatus.IN_PROGRESS.value
            ):
                session_data["status"] = ProvisioningStatus.FAILED.value
                self.stats["failed"] += 1
                self.stats["in_progress"] -= 1
    ...
    async def _generate_device_credentials(
        ...
        credentials = {"device_id": device_id, "auth_methods": []}
        ...
        if session_data.get("generate_certificate", True):
            (
                ...
                key_pem,
                ca_chain,
            ) = await self.certificate_manager.generate_device_certificate(
                device_id=device_id,
                device_type=session_data["device_type"],
                organization=session_data["organization"],
            )
    ...
    async def bulk_provision_devices(
        ...
        for device_info in device_list:
            try:
                ...
                result = await self.provision_device(
                    ...
                    provisioning_token=self.provisioning_sessions[
                        session["session_id"]
                    ]["provisioning_token"],
                )
        ...
        self.logger.info(
            "Bulk provisioning completed", total=len(device_list), successful=successful
        )
    ...
    async def _provisioning_monitor(self) -> None:
        ...
        while True:
            try:
                ...
                if expired_sessions:
                    self.logger.info(
                        "Expired provisioning sessions", count=len(expired_sessions)
                    )

                if sessions_to_remove:
                    self.logger.info(
                        "Cleaned up old provisioning sessions",
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        active_sessions = sum(
            1
            for s in self.provisioning_sessions.values()
            if s["status"]
            in [ProvisioningStatus.PENDING.value, ProvisioningStatus.IN_PROGRESS.value]
        )
