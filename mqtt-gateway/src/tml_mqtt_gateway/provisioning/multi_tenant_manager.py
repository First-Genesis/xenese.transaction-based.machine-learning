...
class MultiTenantManager:
    ...
    def __init__(self, config: TMLGatewayConfig, device_manager: DeviceManager):
        self.config = config
    ...
    async def create_tenant(
        ...
        try:
            ...
            self.logger.error(
                "Failed to create tenant", tenant_id=tenant_id, error=str(e)
            )
            raise

    async def add_device_to_tenant(
        self, tenant_id: str, device_id: str, device_info: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(
                "Added device to tenant", tenant_id=tenant_id, device_id=device_id
            )
            ...
            self.logger.error(
                "Failed to add device to tenant",
                tenant_id=tenant_id,
                device_id=device_id,
                error=str(e),
            )
    ...
    def validate_tenant_topic_access(self, tenant_id: str, topic: str) -> bool:
        """
    ...
    def isolate_message_for_tenant(self, tenant_id: str, topic: str) -> str:
        """
    ...
    async def update_tenant_usage(
        self, tenant_id: str, message_count: int = 0, bandwidth_mbps: float = 0.0
    ) -> None:
        """
        ...
        try:
            ...
            usage["bandwidth_used_mbps"] = max(
                usage["bandwidth_used_mbps"], bandwidth_mbps
            )
            usage["last_updated"] = datetime.utcnow()
            ...
            if usage["bandwidth_used_mbps"] > quota["max_bandwidth_mbps"]:
                self.logger.warning(
                    "Tenant exceeded bandwidth quota",
                    tenant_id=tenant_id,
            ...
            self.logger.error(
                "Failed to update tenant usage", tenant_id=tenant_id, error=str(e)
            )
    ...
    async def suspend_tenant(self, tenant_id: str, reason: str = None) -> None:
        ...
        self.logger.warning("Tenant suspended", tenant_id=tenant_id, reason=reason)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_tenants": sum(
                ...
                if t["status"] == TenantStatus.ACTIVE.value
            ),
            "suspended_tenants": sum(
                1
                for t in self.tenants.values()
                if t["status"] == TenantStatus.SUSPENDED.value
            ),
            "total_devices": sum(
                len(devices) for devices in self.tenant_devices.values()
            ),
            "plans": {
                "trial": sum(1 for t in self.tenants.values() if t["plan"] == "trial"),
                "standard": sum(
                    1 for t in self.tenants.values() if t["plan"] == "standard"
                ),
                "professional": sum(
                    1 for t in self.tenants.values() if t["plan"] == "professional"
                ),
                "enterprise": sum(
                    1 for t in self.tenants.values() if t["plan"] == "enterprise"
                ),
            },
        }
