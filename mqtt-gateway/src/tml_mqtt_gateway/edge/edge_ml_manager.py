...
class EdgeMLManager:
    ...
    async def deploy_model(
        ...
        """
        try:
            self.logger.info("Deploying model to edge",
                           model_id=model_id,
    ...
    async def predict(
        ...
        try:
            ...
            if model_info['status'] != ModelStatus.READY.value:
                raise ValueError(f"Model {model_id} not ready (status: {model_info['status']})")
            ...
            if model_info['model_type'] == ModelType.ANOMALY_DETECTION.value:
                ...
                anomaly_score = model.decision_function(feature_array) if hasattr(model, 'decision_function') else None
                result = {
                ...
                result = {
                    'prediction': int(prediction[0]) if isinstance(prediction[0], (np.integer, int)) else str(prediction[0]),
    ...
    async def update_model(
        ...
        try:
            ...
            self.logger.info("Model update queued",
                           model_id=model_id,
            ...
            self.logger.error("Failed to queue model update",
                            model_id=model_id,
                            error=str(e))
            return False
    ...
    async def remove_model(self, model_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error("Failed to remove model",
                            model_id=model_id,
                            error=str(e))
            return False
    ...
    async def _load_model(self, model_file: Path, model_type: ModelType) -> Any:
        ...
        try:
            ...
            if model_type in [ModelType.ANOMALY_DETECTION, ModelType.CLASSIFICATION, ModelType.REGRESSION]:
    ...
    def _prepare_features(self, features: Dict[str, Any], model_info: Dict[str, Any]) -> np.ndarray:
        """Prepare features for model input"""
        try:
            # Extract feature values in consistent order
            feature_names = model_info.get('metadata', {}).get('feature_names', list(features.keys()))
        ...
        try:
            ...
            if perf['avg_inference_time_ms'] == 0:
                ...
                perf['avg_inference_time_ms'] = (
                    ...
                    (1 - alpha) * perf['avg_inference_time_ms']
                )
            ...
            self.logger.error("Failed to update inference stats",
                            model_id=model_id,
                            error=str(e))
                    ...
                    if model_files:
                        ...
                        try:
                            ...
                            model_info = {
                                ...
                                'deployed_at': datetime.fromtimestamp(latest_file.stat().st_mtime),
                                'inference_count': 0,
                                'performance_metrics': {
                                    'avg_inference_time_ms': 0.0,
                                    'accuracy': 0.0,
                            ...
                            self.logger.error("Failed to load existing model",
                                            model_id=model_id,
                                            error=str(e))
        ...
        while True:
            try:
                ...
                for model_id, model_info in self.deployed_models.items():
                    ...
                    if perf['avg_inference_time_ms'] > self.performance_thresholds['max_inference_time_ms']:
                        self.logger.warning("Model inference time exceeded threshold",
                                          model_id=model_id,
                                          inference_time=perf['avg_inference_time_ms'])
                    
                    # Check if model has been used recently
                    if model_info['last_inference']:
                        time_since_last = datetime.utcnow() - model_info['last_inference']
                        if time_since_last > timedelta(hours=24):
                            self.logger.info("Model inactive for 24 hours",
                                           model_id=model_id)
    ...
    async def _update_processor(self) -> None:
        ...
        while True:
            try:
                ...
                self.logger.info("Processing model update",
                               model_id=model_id,
                               new_version=new_version)
                
                # Update model status
                if model_id in self.deployed_models:
                    self.deployed_models[model_id]['status'] = ModelStatus.UPDATING.value
                ...
                # Load new model
                new_model_instance = await self._load_model(new_model_file, ModelType.CLASSIFICATION)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        total_inferences = sum(info['inference_count'] for info in self.deployed_models.values())
        
        return {
            'deployed_models': len(self.deployed_models),
            'ready_models': sum(1 for info in self.deployed_models.values() if info['status'] == ModelStatus.READY.value),
