...
class CertificateManager:
    ...
    async def _create_intermediate_ca(self) -> None:
        ...
        self.intermediate_ca_cert = x509.CertificateBuilder().subject_name(
            subject
        ).issuer_name(
        ...
        with open(self.ca_dir / "intermediate_ca.key", 'wb') as f:
            f.write(self.intermediate_ca_key.private_bytes(
                encoding=Encoding.PEM,
                format=PrivateFormat.PKCS8,
                encryption_algorithm=NoEncryption()
            ))
    ...
    async def generate_device_certificate(
        ...
        try:
            ...
            subject = x509.Name([
                ...
                x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, f"IoT {device_type.title()}"),
            ...
            ca_chain = (
                self.intermediate_ca_cert.public_bytes(Encoding.PEM) +
            ...
            self.logger.info("Generated device certificate",
                           device_id=device_id,
                           device_type=device_type,
                           serial_number=device_cert.serial_number,
            ...
            self.logger.error("Failed to generate device certificate",
                            device_id=device_id,
                            error=str(e))
            raise
    ...
    async def revoke_certificate(self, serial_number: int, reason: str = "unspecified") -> None:
        """
        ...
        try:
            ...
            self.logger.error("Failed to revoke certificate",
                            serial_number=serial_number,
                            error=str(e))
            raise
    ...
    async def _update_crl(self) -> None:
        ...
        try:
            ...
            self.logger.info("Updated Certificate Revocation List",
    ...
    async def _certificate_monitor(self) -> None:
        ...
        while True:
            try:
                ...
                if expiring_soon:
                    self.logger.warning("Certificates expiring soon",
