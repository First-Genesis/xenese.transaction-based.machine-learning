...
class DeviceAuthenticator:
    ...
    async def authenticate_device(
        ...
        try:
            ...
            return AuthResult(
                success=False, error_message=f"Authentication error: {str(e)}"
            )
    ...
    async def _authenticate_username_password(
        ...
        if not device:
            return AuthResult(
                success=False, device_id=username, error_message="Device not found"
            )
        ...
        if not stored_password or not self._verify_password(password, stored_password):
            return AuthResult(
                success=False, device_id=username, error_message="Invalid credentials"
            )
    ...
    async def _authenticate_x509_certificate(
        self, credentials: Dict[str, Any]
    ) -> AuthResult:
        ...
        if not cert_pem:
            return AuthResult(success=False, error_message="Certificate is required")

        # Validate certificate
        validation_result = await self.certificate_manager.validate_device_certificate(
            cert_pem
        )
    ...
    async def _authenticate_jwt_token(self, credentials: Dict[str, Any]) -> AuthResult:
        ...
        if not token:
            return AuthResult(success=False, error_message="JWT token is required")
        ...
        try:
            ...
            if not device_id:
                return AuthResult(
                    success=False, error_message="Invalid token: missing device_id"
                )
            ...
            if not device:
                return AuthResult(
                    success=False, device_id=device_id, error_message="Device not found"
                )
            ...
            return AuthResult(
                ...
                metadata={
                    "token_issued_at": datetime.utcfromtimestamp(
                        payload.get("iat", 0)
                    ).isoformat(),
                    "device_type": device.get("device_type"),
                },
            ...
            return AuthResult(
                success=False, error_message=f"Invalid JWT token: {str(e)}"
            )
    ...
    async def _authenticate_api_key(self, credentials: Dict[str, Any]) -> AuthResult:
        ...
        if not api_key:
            return AuthResult(success=False, error_message="API key is required")
    ...
    async def _authenticate_hmac_signature(
        self, credentials: Dict[str, Any]
    ) -> AuthResult:
        ...
        try:
            ...
            if (
                abs((datetime.utcnow() - request_time).total_seconds()) > 300
            ):  # 5 minutes
                return AuthResult(
                    success=False,
                    device_id=device_id,
    ...
    async def generate_jwt_token(
        ...
        self.logger.info(
            "Generated JWT token", device_id=device_id, expiry_hours=expiry_hours
        )
    ...
    async def create_api_key(
        ...
        self.logger.info(
            "Created API key", device_id=device_id, expiry_days=expiry_days
        )
    ...
    async def _record_auth_attempt(self, device_id: str, client_ip: str) -> None:
        ...
        self.auth_attempts[device_id] = [
            attempt
            for attempt in self.auth_attempts[device_id]
    ...
    async def _handle_failed_auth(self, device_id: str) -> None:
        ...
        if len(attempts) >= self.max_attempts_per_hour:
            ...
            self.logger.warning(
                "Device locked due to failed attempts",
                device_id=device_id,
