    ...
    def _setup_routes(self) -> None:
        ...
        async def get_device(device_id: str):
            ...
            try:
                ...
                self.logger.error(
                    "Error getting device", device_id=device_id, error=str(e)
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
                )
        ...
        async def list_devices(
            ...
            try:
                devices = await self.gateway.device_manager.list_devices(
                    device_type=device_type, status=status, limit=limit, offset=offset
                )
        ...
        async def update_device_status(
            ...
            try:
                ...
                self.logger.error(
                    "Error updating device status", device_id=device_id, error=str(e)
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
                )
