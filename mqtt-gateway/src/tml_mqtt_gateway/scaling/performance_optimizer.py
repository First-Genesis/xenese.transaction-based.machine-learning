...
class PerformanceOptimizer:
    ...
    async def initialize(self) -> None:
        ...
        try:
            self.logger.info(
                "Initializing performance optimizer", profile=self.current_profile.value
            )
            ...
            self.logger.error(
                "Failed to initialize performance optimizer", error=str(e)
            )
            raise
    ...
    async def optimize_for_devices(self, device_count: int) -> Dict[str, Any]:
        ...
        try:
            ...
            optimized_params["connection_pool_size"] = max(1000, device_count // 100)

            # Worker threads
            # Rule: 1 worker per 1000 devices, min CPU*2, max CPU*8
            cpu_count = multiprocessing.cpu_count()
            optimized_params["worker_threads"] = min(
                max(cpu_count * 2, device_count // 1000), cpu_count * 8
            )
            ...
            optimized_params["max_concurrent_requests"] = min(
                device_count * 2, 100000  # Hard limit
            )
            ...
            self.logger.error(
                "Failed to optimize for devices",
                device_count=device_count,
                error=str(e),
            )
            raise
    ...
    async def set_performance_profile(self, profile: PerformanceProfile) -> None:
        ...
        try:
            ...
            self.logger.error(
                "Failed to set performance profile", profile=profile.value, error=str(e)
            )
            raise
    ...
    async def batch_messages(
        ...
        try:
            ...
            self.logger.error("Failed to batch message", topic=topic, error=str(e))
            return None
    ...
    async def _collect_performance_metrics(self) -> Dict[str, float]:
        ...
        if self.performance_metrics["processing_latency"]:
            latencies = [
                m["latency"] for m in self.performance_metrics["processing_latency"]
            ]
        ...
        if self.performance_metrics["message_throughput"]:
            ...
            metrics["throughput_msg_per_sec"] = sum(
                m["count"] for m in recent_throughput
            ) / len(recent_throughput)
        ...
        if self.performance_metrics["connection_count"]:
            metrics["active_connections"] = self.performance_metrics[
                "connection_count"
            ][-1]["count"]
    ...
    def _calculate_adjustments(
        ...
        if "throughput_msg_per_sec" in current_metrics:
            if (
                ...
                < self.target_metrics["throughput_msg_per_sec"]
            ):
                # Increase batch size to improve throughput
                adjustments["batch_size"] = min(
                    1000, int(self.optimization_params["batch_size"] * 1.2)
                )
                # Increase worker threads
                cpu_count = multiprocessing.cpu_count()
                if self.optimization_params["worker_threads"] < cpu_count * 4:
                    adjustments["worker_threads"] = min(
                        cpu_count * 4, self.optimization_params["worker_threads"] + 2
                    )

        # Check CPU usage
        if (
            current_metrics.get("cpu_usage_percent", 0)
            > self.target_metrics["cpu_usage_percent"]
        ):
    ...
    async def _apply_adjustments(self, adjustments: Dict[str, Any]) -> None:
        ...
        try:
            self.logger.info(
                "Applying performance adjustments", adjustments=adjustments
            )
            ...
            self.logger.error(
                "Failed to apply adjustments", adjustments=adjustments, error=str(e)
            )
        ...
        while True:
            try:
                ...
                self.performance_metrics["cpu_usage"].append(
                    {"timestamp": timestamp, "usage": psutil.cpu_percent()}
                )

                self.performance_metrics["memory_usage"].append(
                    {"timestamp": timestamp, "usage": psutil.virtual_memory().percent}
                )
