...
class TMLMQTTGateway:
    ...
    def __init__(self, config: TMLGatewayConfig = None):
        ...
        self.logger = logger.bind(
            gateway_id=self.config.gateway.gateway_id, component="gateway"
        )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.device_manager = DeviceManager(
                self.config.database, self.config.redis, self.metrics.database
            )
            await self.device_manager.initialize()

            # Initialize Kafka producer
            self.kafka_producer = TMLKafkaProducer(
                self.config.kafka, self.metrics.kafka
            )
            self.kafka_producer.initialize()

            # Initialize MQTT client
            self.mqtt_client = TMLMQTTClient(
                self.config.mqtt, self._handle_mqtt_message, self.metrics.mqtt
            )
            self.mqtt_client.initialize()

            # Initialize API server
            self.api_server = APIServer(self.config.gateway, self, self.metrics)
        ...
        try:
            while self.running:
                try:
                    ...
                    worker_logger.error(
                        "Error in message processing worker", error=str(e)
                    )
        ...
        try:
            ...
            if "telemetry" in topic:
                ...
                return "data" in message or any(
                    key for key in message.keys() if not key.startswith("_")
                )
    ...
    def _extract_device_info(
        ...
        try:
            ...
            self.logger.error("Error extracting device info", topic=topic, error=str(e))
            return None
    ...
    async def _health_monitor(self) -> None:
        ...
        while self.running:
            try:
                ...
                db_healthy = (
                    await self.device_manager.health_check()
                    if self.device_manager
                    else False
                )
