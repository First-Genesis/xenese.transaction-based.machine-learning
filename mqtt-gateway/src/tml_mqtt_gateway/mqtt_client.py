...
class TMLMQTTClient:
    ...
    def subscribe_to_topics(self) -> None:
        ...
        try:
            for topic in self.config.subscribe_topics:
                ...
                if result == mqtt.MQTT_ERR_SUCCESS:
                    self.logger.info(
                        "Subscribed to topic", topic=topic, qos=self.config.qos
                    )
                else:
                    self.logger.error(
    ...
    def _on_disconnect(self, client, userdata, rc) -> None:
        ...
        if rc != 0:
            self.logger.warning(
                "Unexpected disconnection from MQTT broker", result_code=rc
            )
        else:
            self.logger.info("Disconnected from MQTT broker")
    ...
    def _on_message(self, client, userdata, msg) -> None:
        ...
        try:
            ...
            self.logger.error(
                "Error processing MQTT message", topic=msg.topic, error=str(e)
            )
    ...
    def _on_subscribe(self, client, userdata, mid, granted_qos) -> None:
        ...
        self.logger.info(
            "Subscription confirmed", message_id=mid, granted_qos=granted_qos
        )
    ...
    def _process_message_queue(self) -> None:
        ...
        for topic, message_data in self.message_queue:
            try:
                ...
                self.logger.error(
                    "Error processing queued message", topic=topic, error=str(e)
                )
        ...
        try:
            ...
            if self.connected:
                ...
                if result.rc == mqtt.MQTT_ERR_SUCCESS:
                    ...
                    self.logger.error(
                        "Failed to publish message", topic=topic, result=result.rc
                    )
            else:
