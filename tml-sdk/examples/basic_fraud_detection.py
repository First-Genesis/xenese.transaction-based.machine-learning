...
def generate_sample_transaction(is_fraud: bool = False) -> Transaction:
    ...
    if is_fraud:
        ...
        merchant_category_code = random.choice(
            [3, 4, 5, 6]
        )  # 3=grocery, 4=restaurant, 5=retail, 6=pharmacy
...
def main():
    ...
    client = TMLClient(api_url="http://localhost:8000", api_key="demo-key-12345")

    try:
        # Test connection (will work in offline mode too)
        print(f"   Client initialized for: {client.config.api_url}")
        print(
            f"   Connection status: {'Connected' if client.is_connected() else 'Offline mode'}"
        )
        ...
        model = client.models.create(
            ...
            hyperparameters={"l2": 0.01, "intercept_lr": 0.1},
            metadata={
        ...
        print(
            f"   Fraud rate: {sum(1 for t in training_transactions if t.label == 1) / len(training_transactions) * 100:.1f}%"
        )
        ...
        for i, transaction in enumerate(training_transactions):
            ...
            if i > 0:
                try:
                    ...
                    transaction.add_prediction(
                        model.model_id, prediction, confidence=abs(prediction - 0.5) * 2
                    )
            ...
            if (i + 1) % 200 == 0:
                ...
                print(
                    f"   Progress: {i + 1}/{len(training_transactions)} - Accuracy: {accuracy:.1f}%"
                )
        ...
        model.update_metrics(
            {
                ...
                "fraud_rate": sum(1 for t in training_transactions if t.label == 1)
        ...
        print("   " + "-" * 80)
        print(
            "   Transaction ID       | Amount  | Hour | Category    | Pred | Conf | Actual"
        )
        print("   " + "-" * 80)
        ...
        for transaction in test_transactions[-10:]:  # Show last 10
            ...
            # Add prediction to transaction
            transaction.add_prediction(
                model.model_id, prediction, confidence=confidence
            )
            ...
            category_name = category_names.get(
                transaction.features["merchant_category"], "unknown"
            )

            print(
                f"   {transaction.transaction_id[:16]:<16} | "
        ...
        for i in range(5):
            ...
            print(
                f"   Transaction {i+1}: {result} (confidence: {confidence:.2f}) "
