...
class RealTimeProcessor:
    ...
    def process_transaction(self, transaction):
        ...
        if len(self.recent_predictions) >= 10:
            recent_correct = sum(
                1 for p in self.recent_predictions[-50:] if p["correct"]
            )
...
def main():
    ...
    client = TMLClient(api_url="http://localhost:8000", api_key="streaming-demo-key")
    ...
    try:
        ...
        print("   " + "=" * 90)
        print(
            "   Transaction ID | Prediction | Confidence | Actual | Status | Pred(ms) | Learn(ms)"
        )
        print("   " + "=" * 90)
        ...
        try:
            while True:
                ...
                if current_time - last_stats_time >= 10:
                    ...
                    if stats:
                        ...
                        print(
                            f"      Processed: {stats['processed_count']} transactions"
                        )
                        print(
                            f"      Overall Accuracy: {stats['overall_accuracy']:.1%}"
                        )
                        print(f"      Recent Accuracy: {stats['recent_accuracy']:.1%}")
                        print(
                            f"      Avg Prediction Time: {stats['avg_prediction_time_ms']:.2f}ms"
                        )
                        print(
                            f"      Avg Learning Time: {stats['avg_learning_time_ms']:.2f}ms"
                        )
                        print("   " + "=" * 90)
            ...
            if final_stats:
                print(
                    f"   📈 Total Transactions Processed: {final_stats['processed_count']}"
                )
                print(f"   🎯 Overall Accuracy: {final_stats['overall_accuracy']:.1%}")
                print(
                    f"   ⚡ Average Prediction Time: {final_stats['avg_prediction_time_ms']:.2f}ms"
                )
                print(
                    f"   🧠 Average Learning Time: {final_stats['avg_learning_time_ms']:.2f}ms"
                )
                print(
                    f"   📊 Throughput: {final_stats['processed_count'] / 60:.1f} transactions/minute"
                )
