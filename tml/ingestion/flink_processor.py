...
class TransactionModelProcessor(KeyedProcessFunction):
    ...
    def open(self, runtime_context: RuntimeContext):
        ...
        self.model_state_descriptor = ValueStateDescriptor(
            "model_state",
            Types.STRING()  # JSON serialized ModelState
        )
        ...
        self.feature_history_descriptor = ListStateDescriptor(
            "feature_history",
            Types.STRING()  # JSON serialized features
        )
        self.feature_history = runtime_context.get_list_state(self.feature_history_descriptor)
    ...
    def _process_transaction(self, event: TransactionEvent, 
                           state: ModelState, ctx) -> Dict[str, Any]:
    ...
    def _update_model(self, features: Dict[str, Any], target: Any, 
                     state: ModelState) -> float:
        ...
        if state.update_count == 0:
            ...
            new_accuracy = (state.accuracy * state.update_count + (1.0 if correct else 0.0)) / total_updates
