...
class SpatialModelProcessor(KeyedProcessFunction):
    ...
    def process_element(self, transaction: str, ctx, out):
        ...
        try:
            ...
            model_id = tx_data.get('model_id', f"model_{tx_data.get('transaction_id', 'unknown')}")
            ...
            if state.transaction_count % 100 == 0:
                logger.info(f"Model {model_id}: Processed {state.transaction_count} transactions, "
    ...
    def _create_model_with_inheritance(self, model_id: str, tx_data: Dict) -> ModelStateData:
...
class FlinkTMLPipeline:
    ...
    def build_pipeline(self):
        ...
        keyed_stream = transaction_stream \
            .key_by(lambda tx: self._extract_model_id(tx))
        
        # Process with spatial model inheritance
        processed_stream = keyed_stream \
            .process(SpatialModelProcessor()) \
            .name("Spatial Model Processing")
    ...
    def _extract_model_id(self, transaction: str) -> str:
        ...
        try:
            tx_data = json.loads(transaction)
            return tx_data.get('model_id', f"model_{tx_data.get('transaction_id', 'unknown')}")
        except:
