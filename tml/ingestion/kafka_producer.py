...
class TransactionProducer:
    ...
    def send_transaction(self, event: TransactionEvent) -> bool:
        ...
        try:
            ...
            logger.debug(f"Sent transaction {event.transaction_id} to topic {self.transaction_topic}")
            return True
    ...
    def send_model_update(self, model_id: str, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            future = self.producer.send(
                topic=self.model_updates_topic,
                key=model_id,
                value=update_event
            )
            ...
            logger.debug(f"Sent model update for {model_id} to topic {self.model_updates_topic}")
            return True
...
class TransactionEventGenerator:
    ...
    def generate_event(self, event_type: Optional[str] = None, 
                      user_id: Optional[str] = None) -> TransactionEvent:
    ...
    def _generate_features(self, event_type: str) -> Dict[str, Any]:
        ...
        if event_type == 'purchase':
            ...
            base_features.update({
                'page_type': random.choice(['product', 'category', 'home', 'search']),
