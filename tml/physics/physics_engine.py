...
class PhysicsEngine:
    ...
    def add_constraint(self, constraint: PhysicsConstraint, group: Optional[str] = None):
    ...
    def validate_transaction(self, transaction_data: Dict[str, Any], 
        ...
        results["summary"] = {
            ...
            "compliance_rate": 1.0 - (len(results["violations"]) / len(constraint_names))
        }
        
        return results
        
    def validate_model_inheritance(self, parent_constraints: Dict[str, Any], 
                                 child_transaction: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        validation_result["valid"] = validation_result["valid"] and inheritance_checks["valid"]
        
        return validation_result
        
    def _check_inheritance_consistency(self, parent_data: Dict[str, Any], 
        ...
        for key, child_value in child_data.items():
            if key in parent_data:
                ...
                if isinstance(parent_value, (int, float)) and isinstance(child_value, (int, float)):
                    change_ratio = abs(child_value - parent_value) / max(abs(parent_value), 1e-10)
