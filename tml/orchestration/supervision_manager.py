...
from .actor_system import (
    ActorSystem, Actor, ActorMessage, ActorState, 
...
class SupervisionManager:
    ...
    def register_actor_supervision(self, actor_id: str, parent_id: Optional[str] = None, 
        ...
        logger.info(f"Registered actor {actor_id} for supervision with policy: {policy.strategy.value}")
    
    async def handle_actor_failure(self, actor_id: str, error: Exception, 
                                 fault_type: FaultType = FaultType.ACTOR_CRASH) -> bool:
        ...
        logger.error(f"Actor failure detected: {actor_id}, Type: {fault_type.value}, Error: {error}")
        
        # Get supervision policy
        policy = self.actor_policies.get(actor_id, SupervisionPolicy(SupervisionStrategy.RESTART))
        
        # Check circuit breaker
        if actor_id in self.circuit_breakers:
            circuit_breaker = self.circuit_breakers[actor_id]
            if circuit_breaker.is_open():
                logger.warning(f"Circuit breaker open for {actor_id}, skipping recovery")
                fault_event.recovery_action = RecoveryAction.CIRCUIT_BREAK
                self.recovery_stats['circuit_breaks'] += 1
                return False
        
        # Determine recovery action
        recovery_action = await self._determine_recovery_action(actor_id, fault_event, policy)
        fault_event.recovery_action = recovery_action
        
        # Execute recovery
        recovery_successful = await self._execute_recovery(actor_id, fault_event, policy)
        fault_event.recovery_successful = recovery_successful
        
        if recovery_successful:
            self.recovery_stats['successful_recoveries'] += 1
            logger.info(f"Successfully recovered actor {actor_id} using {recovery_action.value}")
        else:
    ...
    async def _determine_recovery_action(self, actor_id: str, fault_event: FaultEvent, 
                                       policy: SupervisionPolicy) -> RecoveryAction:
    ...
    async def _execute_recovery(self, actor_id: str, fault_event: FaultEvent, 
                              policy: SupervisionPolicy) -> bool:
        ...
        try:
            if fault_event.recovery_action == RecoveryAction.RESTART_ACTOR:
                ...
                logger.warning(f"Unknown recovery action: {fault_event.recovery_action}")
                return False
    ...
    async def _restart_actor(self, actor_id: str, policy: SupervisionPolicy) -> bool:
        ...
        try:
            ...
            recent_faults = self._get_recent_faults(actor_id, policy.retry_window)
            backoff_delay = min(
                policy.backoff_multiplier ** len(recent_faults),
                policy.max_backoff
            )
            
            if backoff_delay > 0:
                logger.info(f"Applying backoff delay of {backoff_delay}s for {actor_id}")
    ...
    async def _restart_subtree(self, root_actor_id: str) -> bool:
        ...
        # Restart root actor
        policy = self.actor_policies.get(root_actor_id, SupervisionPolicy(SupervisionStrategy.RESTART))
    ...
    async def _escalate_to_parent(self, actor_id: str, fault_event: FaultEvent) -> bool:
        ...
        return await self.handle_actor_failure(
            parent_id, 
            Exception(escalation_fault.error_message),
    ...
    def _get_recent_faults(self, actor_id: str, window_seconds: float) -> List[FaultEvent]:
        ...
        return [
            fault for fault in self.fault_history[actor_id]
    ...
    async def _recover_from_timeout(self, actor_id: str, fault_event: FaultEvent) -> bool:
        """Recover from message timeout"""
        # For timeouts, try restarting the actor
        policy = self.actor_policies.get(actor_id, SupervisionPolicy(SupervisionStrategy.RESTART))
        return await self._restart_actor(actor_id, policy)
    
    async def _recover_from_resource_exhaustion(self, actor_id: str, fault_event: FaultEvent) -> bool:
    ...
    async def _recover_from_network_partition(self, actor_id: str, fault_event: FaultEvent) -> bool:
    ...
    async def _recover_from_data_corruption(self, actor_id: str, fault_event: FaultEvent) -> bool:
        """Recover from data corruption"""
        # For data corruption, restart the entire subtree
        return await self._restart_subtree(actor_id)
    
    async def _recover_from_service_failure(self, actor_id: str, fault_event: FaultEvent) -> bool:
            ...
            try:
                ...
                if actor_id in self.actor_system.actors:
                    ...
                    if actor.state in [ActorState.FAILED, ActorState.STOPPED]:
                        logger.warning(f"Actor {actor_id} is in unhealthy state: {actor.state}")
    ...
    def _calculate_tree_depth(self) -> int:
        ...
        max_depth = 0
        root_actors = [
            actor_id for actor_id in self.actor_policies.keys()
            if actor_id not in self.parent_map
        ]
