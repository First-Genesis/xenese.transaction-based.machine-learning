...
class FaultTolerantActorBridge:
    ...
    def __init__(
        ...
        self.cluster_manager = EnhancedClusterManager(
            self.actor_system, self.cluster_config
        )
        ...
        logger.info(
            f"Fault-Tolerant Actor Bridge initialized: {self.actor_system.node_id}"
        )
    ...
    async def _create_supervised_actors(self):
        ...
        for i in range(self.parallelism):
            ...
            self.supervision_manager.register_actor_supervision(
                actor_id, parent_id=None, policy=supervision_policy  # Root level
            )

            self.transaction_processors.append(actor_id)
            logger.info(
                f"✅ Created supervised transaction processor {actor_id} on node {node_id}"
            )
        ...
        for service_id, service_class in critical_services:
            ...
            self.supervision_manager.register_actor_supervision(
                service_id, parent_id=None, policy=critical_policy
            )
        ...
        while True:
            try:
                ...
                for topic_partition, records in messages.items():
                    for record in records:
                        try:
                            ...
                            logger.error(
                                f"Error processing individual transaction: {e}"
                            )
                ...
                if consecutive_errors >= max_consecutive_errors:
                    ...
                    await asyncio.sleep(
                        min(consecutive_errors * 2, 60)
                    )  # Exponential backoff
                    consecutive_errors = 0  # Reset after backoff
                else:
                    await asyncio.sleep(1)

    async def _process_transaction_with_ft(
        self, transaction: Dict, processor_index: int
    ) -> Optional[FaultTolerantProcessingResult]:
        ...
        try:
            ...
            transaction_id = transaction.get(
                "transaction_id", f"ft_tx_{int(time.time() * 1000)}"
            )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if routed:
                        ...
                        if (
                            processor_actor_id
                            in self.supervision_manager.circuit_breakers
                        ):
                            self.supervision_manager.circuit_breakers[
                                processor_actor_id
                            ].record_success()
                    ...
                    await self.supervision_manager.handle_actor_failure(
                        processor_actor_id, e, FaultType.ACTOR_CRASH
                    )
            ...
            cb_state = "closed"
            if processor_actor_id in self.supervision_manager.circuit_breakers:
                cb_state = self.supervision_manager.circuit_breakers[
                    processor_actor_id
                ].state.value

            # Get processing node
            processing_node = self.cluster_manager.actor_distribution.get(
                processor_actor_id, self.actor_system.node_id
            )
    ...
    async def _handle_transaction_failure(self, transaction: Dict, error: Exception):
        ...
        # Log the failure
        logger.error(
            f"Transaction failure: {transaction.get('transaction_id', 'unknown')} - {error}"
        )
                ...
                self.ft_stats.update(
                    {
                        ...
                        "total_faults_last_hour": supervision_status[
                            "total_faults_last_hour"
                        ],
                    }
                )
    ...
    async def _log_ft_stats(self):
        ...
        logger.info(
            f"  • Successful Recoveries: {supervision_status['recovery_stats']['successful_recoveries']}"
        )
        logger.info(
            f"  • Failed Recoveries: {supervision_status['recovery_stats']['failed_recoveries']}"
        )
        logger.info(
            f"  • Circuit Breaker Trips: {self.ft_stats['circuit_breaker_trips']}"
        )
        logger.info(
            f"  • Healthy Actors: {supervision_status['healthy_actors']}/{supervision_status['supervised_actors']}"
        )
    ...
    async def simulate_fault(
        self, actor_id: str, fault_type: FaultType = FaultType.ACTOR_CRASH
    ):
        """Simulate a fault for testing purposes"""
        logger.info(f"🧪 Simulating {fault_type.value} fault for {actor_id}")

        await self.supervision_manager.handle_actor_failure(
            actor_id, Exception(f"Simulated {fault_type.value} fault"), fault_type
        )
...
async def main():
    ...
    cluster_config = ClusterConfig(
        strategy=ClusterStrategy.LOAD_BALANCED, max_nodes=3, auto_scaling=True
    )

    bridge = FaultTolerantActorBridge(cluster_config=cluster_config, parallelism=3)
