...
class EnhancedClusterManager:
    ...
    def __init__(self, actor_system: ActorSystem, config: ClusterConfig = None):
        ...
        logger.info(f"Enhanced Cluster Manager initialized with strategy: {self.config.strategy.value}")
    ...
    async def create_distributed_actor(self, actor_class, actor_id: str, **kwargs) -> str:
        ...
        if target_node == self.actor_system.node_id:
            # Create locally
            actor_ref = await self.actor_system.create_actor(actor_class, actor_id, **kwargs)
            ...
            # Create remotely
            await self._create_remote_actor(target_node, actor_class, actor_id, **kwargs)
    ...
    def _round_robin_selection(self) -> str:
        ...
        healthy_nodes = [node_id for node_id, node in self.nodes.items() if node.is_healthy()]
    ...
    async def _update_node_loads(self):
        ...
        nodes_data = await self.actor_system.redis_client.hgetall("enhanced_cluster_nodes")
    ...
    async def _create_remote_actor(self, target_node: str, actor_class, actor_id: str, **kwargs):
    ...
    async def route_message_to_actor(self, message: ActorMessage) -> bool:
        ...
        if self.actor_system.redis_client:
            target_node = await self.actor_system.redis_client.get(f"actor_location:{actor_id}")
            if target_node:
    ...
    async def _handle_node_failures(self, failed_nodes: List[str]):
        ...
        for node_id in failed_nodes:
            ...
            failed_actors = [
                actor_id for actor_id, assigned_node in self.actor_distribution.items()
            ...
            logger.info(f"Redistributed {len(failed_actors)} actors from failed node {node_id}")
    ...
    async def _perform_load_balancing(self):
        ...
        for overloaded_node in overloaded:
            ...
            # Find actors to move
            actors_to_move = [
                actor_id for actor_id, node_id in self.actor_distribution.items()
