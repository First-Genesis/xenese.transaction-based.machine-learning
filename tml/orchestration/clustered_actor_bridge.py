...
class ClusteredActorBridge:
    ...
    async def _create_distributed_actors(self):
        ...
        await self.cluster_manager.create_distributed_actor(
            InheritanceCoordinatorActor, inheritance_actor_id
        )

        physics_actor_id = "clustered_physics_validator"
        await self.cluster_manager.create_distributed_actor(
            PhysicsValidatorActor, physics_actor_id
        )
    ...
    async def _process_transaction_clustered(
        self, transaction: Dict, processor_index: int
    ) -> Optional[ClusteredProcessingResult]:
        """Process transaction through clustered actors"""
        start_time = time.time()
        cluster_hops = 0
        ...
        try:
            ...
            transaction_id = transaction.get(
                "transaction_id", f"clustered_tx_{int(time.time() * 1000)}"
            )
            ...
            tx_data = TransactionData(
                ...
                metadata={
                    ...
                    "cluster_node": self.actor_system.node_id,
                },
            )
            ...
            if routed:
                ...
                if processor_actor_id in self.cluster_manager.actor_distribution:
                    target_node = self.cluster_manager.actor_distribution[
                        processor_actor_id
                    ]
    ...
    async def _log_cluster_stats(self):
        ...
        logger.info(
            f"  • Cluster Nodes: {cluster_status['metrics']['healthy_nodes']}/{cluster_status['metrics']['total_nodes']}"
        )
        logger.info(f"  • Local Messages: {self.stats['local_messages']}")
        logger.info(f"  • Remote Messages: {self.stats['remote_messages']}")
        logger.info(
            f"  • Load Balance Events: {self.cluster_manager.cluster_metrics['load_balance_operations']}"
        )
...
async def main():
    ...
    bridge = ClusteredActorBridge(cluster_config=cluster_config, parallelism=4)
