...
class TransactionModelActor(BaseActor):
    ...
    async def _handle_transaction_processing(self, message: TransactionProcessingMessage):
        """Handle processing of new transactions"""
        logger.info(f"Model {self.model_id} processing transaction {message.transaction_id}")
    ...
    async def _learn_from_transaction(self, transaction_data: Dict[str, Any]):
        ...
        self.metadata["transactions_processed"] = self.metadata.get("transactions_processed", 0) + 1
...
class ModelSupervisorActor(BaseActor):
    ...
    async def _spawn_model(self, message: Dict[str, Any]):
        ...
        if "response_queue" in message:
            await message["response_queue"].put({"status": "success", "model_id": model_id})
...
class TMLActorSystem:
    ...
    async def spawn_transaction_model(self, transaction_id: str, parent_model_id: Optional[str] = None, 
    ...
    async def process_transaction(self, model_id: str, transaction_data: Dict[str, Any]):
...
async def main():
    ...
    try:
        ...
        model_2 = await system.spawn_transaction_model("txn_002", parent_model_id=model_1)
        print(f"Created model: {model_2}")
        
        # Process some transactions
        await system.process_transaction(model_1, {
