...
class KafkaConfig(BaseSettings):
    ...
    transaction_topic: str = Field(default="transactions", env="KAFKA_TRANSACTION_TOPIC")
    model_updates_topic: str = Field(default="model_updates", env="KAFKA_MODEL_UPDATES_TOPIC")
...
class MLflowConfig(BaseSettings):
    ...
    experiment_name: str = Field(default="tml_experiments", env="MLFLOW_EXPERIMENT_NAME")
...
class ModelConfig(BaseSettings):
    ...
    base_model_type: str = Field(default="river", env="MODEL_BASE_TYPE")  # river, vowpal_wabbit
    max_models_in_memory: int = Field(default=10000, env="MODEL_MAX_IN_MEMORY")
    model_compression_enabled: bool = Field(default=True, env="MODEL_COMPRESSION_ENABLED")
    ewc_lambda: float = Field(default=0.4, env="MODEL_EWC_LAMBDA")  # Elastic Weight Consolidation
...
class ServingConfig(BaseSettings):
    ...
    max_concurrent_queries: int = Field(default=1000, env="SERVING_MAX_CONCURRENT_QUERIES")
...
class MonitoringConfig(BaseSettings):
    ...
    drift_check_interval: int = Field(default=300, env="MONITORING_DRIFT_CHECK_INTERVAL")  # seconds
