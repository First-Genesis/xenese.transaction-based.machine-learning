...
class ModelMetrics:
    ...
    def update_accuracy(self, y_true: Any, y_pred: Any):
        ...
        if y_true == y_pred:
            self.accuracy = (
                self.accuracy * (self.total_predictions - 1) + 1.0
            ) / self.total_predictions
        else:
            self.accuracy = (
                self.accuracy * (self.total_predictions - 1)
            ) / self.total_predictions
...
class RiverTransactionModel(BaseTransactionModel):
    ...
    def _convert_features_to_numeric(
    ...
    def set_state(self, state: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if "metrics" in state:
                ...
                self.metrics.total_predictions = metrics_data.get(
                    "total_predictions", 0
                )
                ...
                self.metrics.last_updated = metrics_data.get(
                    "last_updated", time.time()
                )
                self.metrics.creation_time = metrics_data.get(
...
class TransactionModel:
    ...
    def _inherit_from_parent(self, parent_model: "TransactionModel") -> None:
        ...
        try:
            ...
            logger.info(
                f"Model {self.model_id} inherited from parent {self.parent_model_id}"
            )
...
class ModelFactory:
    ...
    def create_base_model(model_type: str = "logistic_regression") -> TransactionModel:
        ...
        return TransactionModel(transaction_context=base_context, model_type=model_type)
