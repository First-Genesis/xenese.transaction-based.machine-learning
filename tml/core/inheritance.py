...
class SimilarityCalculator:
    ...
    def __init__(self, spatial_weight: float = 0.4, temporal_weight: float = 0.3, 
                 domain_weight: float = 0.2, feature_weight: float = 0.1):
        self.spatial_weight = spatial_weight
        self.temporal_weight = temporal_weight
        self.domain_weight = domain_weight
        self.feature_weight = feature_weight
    
    def calculate_similarity(self, context1: SpatialContext, 
                           context2: SpatialContext) -> float:
        """
...
class SpatialInheritanceEngine:
    ...
    def __init__(self, max_parents: int = 5, similarity_threshold: float = 0.3,
    ...
    def find_parent_models(self, context: SpatialContext, 
                          exclude_models: Optional[List[str]] = None) -> List[Tuple[str, float]]:
        """
    ...
    def _find_spatial_neighbors(self, context: SpatialContext, 
                              exclude_models: List[str]) -> List[Tuple[str, float]]:
    ...
    def calculate_inheritance_weights(self, parent_similarities: List[Tuple[str, float]]) -> Dict[str, float]:
        """
        ...
        exp_sims = np.exp(np.array(similarities) - np.max(similarities))  # Numerical stability
...
class AdaptiveInheritance:
    ...
    def update_inheritance_success(self, child_model_id: str, parent_model_ids: List[str], 
        ...
        for parent_id in parent_model_ids:
            ...
            new_success = current_success + self.learning_rate * (performance_improvement - current_success)
    ...
    def get_adaptive_similarity_threshold(self, base_threshold: float = 0.3) -> float:
        ...
        if avg_success > 0.7:
            return base_threshold * 0.8  # Lower threshold if inheritance is working well
        elif avg_success < 0.3:
            return base_threshold * 1.2  # Higher threshold if inheritance is not working
        else:
            return base_threshold
...
class SpatialInheritanceCoordinator:
    ...
    def process_inheritance(self, model_id: str, context: SpatialContext) -> Dict[str, Any]:
        """
    ...
    def get_inheritance_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_parents_per_model': np.mean([
                len(info['parents']) for info in self.inheritance_history.values()
            ]) if self.inheritance_history else 0,
            'inheritance_success_rates': dict(self.adaptive.inheritance_success),
            'spatial_index_size': len(self.engine.model_ids) if self.engine.spatial_index else 0
        }
