...
class InheritanceAwareObjective:
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            # Combined objective
            combined_score = (
                (1 - self.inheritance_weight) * base_score + 
                self.inheritance_weight * inheritance_score
            )
...
class RiverMLHyperparameterOptimizer:
    ...
    def define_river_ml_search_space(self, model_type: str) -> List[HyperparameterSpace]:
        ...
        if model_type == "adaptive_random_forest":
            return [
                ...
                HyperparameterSpace("metric", "categorical", choices=["gini", "hellinger"]),
                HyperparameterSpace("drift_detector", "categorical", choices=["adwin", "ddm", "eddm"]),
                HyperparameterSpace("warning_detector", "categorical", choices=["adwin", "ddm", None])
            ]
            ...
            return [
                ...
                HyperparameterSpace("leaf_prediction", "categorical", choices=["mc", "nb", "nba"]),
            ...
            return [
                ...
                HyperparameterSpace("loss", "categorical", choices=["log", "hinge", "perceptron"]),
            ...
            return [
                ...
                HyperparameterSpace("loss", "categorical", choices=["hinge", "squared_hinge"])
            ]
    ...
    def create_river_ml_objective(self, 
                                model_type: str,
                                model_factory: Callable,
                                X_train: np.ndarray,
                                y_train: np.ndarray,
                                inheritance_candidates: List[Dict] = None) -> InheritanceAwareObjective:
        ...
        def evaluate_inheritance(model, params: Dict[str, Any]) -> float:
            ...
            for candidate in inheritance_candidates:
                ...
                param_similarity = self._compute_parameter_similarity(params, candidate.get('params', {}))
    ...
    def _compute_parameter_similarity(self, params1: Dict[str, Any], params2: Dict[str, Any]) -> float:
    ...
    def optimize_hyperparameters(self, 
                                ...
                                custom_search_space: List[HyperparameterSpace] = None) -> OptimizationResult:
        """
        ...
        # Define search space
        search_space = custom_search_space or self.define_river_ml_search_space(model_type)
        ...
        study = optuna.create_study(
            direction=self.config.study_direction,
            sampler=sampler,
            pruner=pruner
        )
            ...
            for space in search_space:
                if space.param_type == "int":
                    params[space.param_name] = trial.suggest_int(
                        space.param_name, int(space.low), int(space.high),
                        step=int(space.step) if space.step else 1
                    )
                elif space.param_type == "float":
                    params[space.param_name] = trial.suggest_float(
                        space.param_name, space.low, space.high,
                        log=space.log, step=space.step
                    )
        ...
        try:
            ...
            inheritance_improvement = ((best_score - baseline_score) / max(abs(baseline_score), 1e-8)) * 100
            ...
            self.optimization_stats['avg_improvement'] = (
                (self.optimization_stats['avg_improvement'] * (self.optimization_stats['successful_optimizations'] - 1) + 
                 inheritance_improvement) / self.optimization_stats['successful_optimizations']
            )
            self.optimization_stats['total_optimization_time'] += optimization_time
            
            logger.info(f"✅ Optimization completed for {model_id}: "
    ...
    async def optimize_multiple_models(self, 
                                     optimization_tasks: List[Dict[str, Any]]) -> Dict[str, OptimizationResult]:
        """Optimize hyperparameters for multiple models in parallel"""
        
        logger.info(f"Starting parallel optimization for {len(optimization_tasks)} models")
        ...
        logger.info(f"✅ Parallel optimization completed: {len(results)}/{len(optimization_tasks)} successful")
    ...
    def get_optimization_insights(self, model_id: str) -> Dict[str, Any]:
        ...
        insights = {
            ...
            'convergence_analysis': {
                'trials_to_best': next(i for i, v in enumerate(result.convergence_history) if v == result.best_score),
                'improvement_rate': np.diff(result.convergence_history[:10]).mean() if len(result.convergence_history) > 10 else 0,
                'stability_score': 1.0 - np.std(result.convergence_history[-10:]) / np.mean(result.convergence_history[-10:]) if len(result.convergence_history) >= 10 else 0
            },
            'trial_statistics': {
            ...
            'optimization_efficiency': {
                ...
                'improvement_per_second': result.inheritance_improvement / result.optimization_time,
    ...
    def save_optimization_state(self, filepath: str):
        ...
        state = {
            'optimization_results': {k: asdict(v) for k, v in self.optimization_results.items()},
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_optimization_time': self.optimization_stats['total_optimization_time'],
