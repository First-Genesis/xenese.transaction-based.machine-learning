...
# TML imports
from ..core.production_processor import ProductionTransactionProcessor, ProductionConfig, ProcessingResult
...
class TMLClient:
    ...
    def __init__(self, config: TMLConfig):
        ...
        if config.processing_mode in [ProcessingMode.LOCAL, ProcessingMode.HYBRID]:
            if config.local_config:
                self.local_processor = ProductionTransactionProcessor(config.local_config)
            else:
                logger.warning("Local processing requested but no local_config provided")
    ...
    async def initialize(self):
        ...
        logger.info(f"TML Client initialized in {self.config.processing_mode.value} mode")
    ...
    async def process_transaction(self, transaction_data: Dict[str, Any]) -> ProcessingResult:
        """
        ...
        try:
            ...
            if self.config.processing_mode == ProcessingMode.LOCAL:
                ...
                raise ProcessingError(f"Unsupported processing mode: {self.config.processing_mode}")
    ...
    async def _process_local(self, transaction_data: Dict[str, Any]) -> ProcessingResult:
    ...
    async def _process_remote(self, transaction_data: Dict[str, Any]) -> ProcessingResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise ProcessingError(f"API request failed: {response.status_code} - {response.text}")
                
        except httpx.RequestError as e:
            raise ProcessingError(f"Network error: {e}")
    
    async def _process_hybrid(self, transaction_data: Dict[str, Any]) -> ProcessingResult:
    ...
    async def _process_distributed(self, transaction_data: Dict[str, Any]) -> ProcessingResult:
    ...
    async def process_batch(self, transactions: List[Dict[str, Any]]) -> List[ProcessingResult]:
        """
    ...
    async def get_model_info(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            response = await self.http_client.get(
                f"{self.config.api_base_url}/api/v1/models/{model_id}",
                headers=headers
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.local_processor:
            try:
                ...
                health['components']['local_processor'] = {'status': 'unhealthy', 'error': str(e)}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                health['components']['remote_api'] = {'status': 'unhealthy', 'code': response.status_code}
                health['status'] = 'degraded'
        except Exception as e:
            health['components']['remote_api'] = {'status': 'unhealthy', 'error': str(e)}
            health['status'] = 'degraded'
...
async def process_single_transaction(transaction_data: Dict[str, Any], 
                                   config: TMLConfig) -> ProcessingResult:
