...
class StatisticalDriftTests:
    ...
    @staticmethod
    def kolmogorov_smirnov_test(reference: np.ndarray, current: np.ndarray) -> DriftTestResult:
        ...
        try:
            ...
            ci_lower = statistic - 1.96 * np.sqrt(statistic * (1 - statistic) / min(n1, n2))
            ci_upper = statistic + 1.96 * np.sqrt(statistic * (1 - statistic) / min(n1, n2))
            
            interpretation = f"KS statistic: {statistic:.4f}, " + \
                           ("Significant drift detected" if is_significant else "No significant drift")
    ...
    @staticmethod
    def mann_whitney_u_test(reference: np.ndarray, current: np.ndarray) -> DriftTestResult:
        ...
        try:
            ...
            interpretation = f"Mann-Whitney U: {statistic:.4f}, " + \
                           ("Significant rank difference detected" if is_significant else "No significant rank difference")
    ...
    @staticmethod
    def anderson_darling_test(reference: np.ndarray, current: np.ndarray) -> DriftTestResult:
        ...
        try:
            ...
            critical_value = critical_values[2] if len(critical_values) > 2 else critical_values[-1]
            ...
            interpretation = f"Anderson-Darling: {statistic:.4f}, " + \
                           ("Significant distribution difference detected" if is_significant else "No significant distribution difference")
            
            return DriftTestResult(
                test_name="Anderson-Darling",
                test_statistic=statistic,
                p_value=p_value if p_value is not None else (0.01 if is_significant else 0.1),
                critical_value=critical_value,
                is_significant=is_significant,
                effect_size=effect_size,
                confidence_interval=(ci_lower, ci_upper),
    ...
    @staticmethod
    def population_stability_index(reference: np.ndarray, current: np.ndarray, bins: int = 10) -> DriftTestResult:
    ...
    @staticmethod
    def chi_square_test(reference: np.ndarray, current: np.ndarray, bins: int = 10) -> DriftTestResult:
        """Chi-square test for categorical or binned continuous data"""
        try:
            # Create bins for continuous data
            if len(np.unique(reference)) > bins:
                bin_edges = np.percentile(np.concatenate([reference, current]), 
            ...
            interpretation = f"Chi-square: {chi2_stat:.4f}, " + \
                           ("Significant categorical distribution change" if is_significant else "No significant categorical change")
...
class AdvancedDriftDetector:
    ...
    def __init__(self, config: DriftMonitoringConfig = None):
        ...
        if self.config.statistical_tests is None:
            self.config.statistical_tests = ['kolmogorov_smirnov', 'mann_whitney_u', 'population_stability_index']
    ...
    def set_reference_data(self, model_id: str, reference_data: Dict[str, np.ndarray]):
        ...
        logger.info(f"Reference data set for model {model_id}: {len(reference_data)} features")
    
    async def detect_drift(self, 
                          model_id: str,
                          current_data: Dict[str, np.ndarray],
                          drift_type: DriftType = DriftType.COVARIATE_DRIFT) -> DriftDetectionResult:
        """
        ...
        logger.info(f"Starting drift detection for model {model_id}, type: {drift_type.value}")
        ...
        aggregated_result.recommendations = self._generate_recommendations(aggregated_result)
        ...
        self.detection_stats['avg_detection_time'] = (
            (self.detection_stats['avg_detection_time'] * (self.detection_stats['total_detections'] - 1) + 
             detection_time) / self.detection_stats['total_detections']
        )
    ...
    async def _run_feature_tests(self, 
                                reference: np.ndarray, 
                                current: np.ndarray,
                                feature_name: str) -> List[DriftTestResult]:
        ...
        valid_results = [result for result in test_results if isinstance(result, DriftTestResult)]
        
        executor.shutdown(wait=False)
        
        return valid_results
    
    def _validate_data_compatibility(self, 
                                   reference_data: Dict[str, np.ndarray],
                                   current_data: Dict[str, np.ndarray]) -> bool:
        ...
        for feature in common_features:
            ...
            if not (np.issubdtype(ref_data.dtype, np.number) and 
                   np.issubdtype(cur_data.dtype, np.number)):
    ...
    def _aggregate_test_results(self, 
                              ...
                              feature_results: Dict[str, List[DriftTestResult]]) -> DriftDetectionResult:
        ...
        significant_tests = [result for result in all_test_results if result.is_significant]
        is_drift_detected = len(significant_tests) > len(all_test_results) * 0.5  # Majority rule
        
        # Calculate drift magnitude (average effect size)
        effect_sizes = [result.effect_size for result in all_test_results if result.effect_size > 0]
        drift_magnitude = np.mean(effect_sizes) if effect_sizes else 0.0
        
        # Determine drift severity
        drift_severity = self._determine_drift_severity(drift_magnitude, overall_p_value)
        
        # Calculate feature-level drift scores
        feature_drift_scores = {}
        for feature_name, feature_tests in feature_results.items():
            if feature_tests:
                feature_p_values = [test.p_value for test in feature_tests if test.p_value > 0]
                if feature_p_values:
    ...
    def _determine_drift_severity(self, drift_magnitude: float, p_value: float) -> DriftSeverity:
    ...
    def _calculate_statistical_power(self, 
                                   reference_data: Dict[str, np.ndarray],
                                   current_data: Dict[str, np.ndarray]) -> float:
        ...
        for feature_name in reference_data.keys():
            if feature_name in current_data:
                ...
                if pooled_std > 0:
                    effect_size = abs(np.mean(cur_data) - np.mean(ref_data)) / pooled_std
                    ...
                    power = 1 - stats.norm.cdf(stats.norm.ppf(1 - self.config.significance_level/2) - delta)
                    powers.append(power)
    ...
    def _generate_recommendations(self, result: DriftDetectionResult) -> List[str]:
        ...
        top_drifted_features = sorted(
            result.feature_drift_scores.items(), 
            key=lambda x: x[1]
        )[:3]  # Top 3 most drifted features
    ...
    def get_drift_trends(self, model_id: str, lookback_hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            'latest_detection': recent_detections[-1].timestamp if recent_detections else None
        }
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        drift_rate = (
            ...
            max(1, self.detection_stats['total_detections']) * 100
        )
    ...
    def export_detection_results(self, model_id: str = None) -> List[Dict[str, Any]]:
        ...
        if model_id:
            results = [result for result in self.detection_history if result.model_id == model_id]
        else:
