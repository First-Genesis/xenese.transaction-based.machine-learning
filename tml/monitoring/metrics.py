...
class MetricsCollector:
    ...
    def record_prediction(self, model_id: str, processing_time_ms: float, 
                         success: bool = True):
        ...
        with self.lock:
            ...
            self.prediction_duration.labels(model_id=model_id).observe(processing_time_ms / 1000.0)
    ...
    def record_learning_update(self, model_id: str, processing_time_ms: float, 
                              success: bool = True):
        ...
        with self.lock:
            ...
            self.model_metrics[model_id].update_learning_metrics(processing_time_ms, success)
            ...
            if success:
                self.learning_duration.labels(model_id=model_id).observe(processing_time_ms / 1000.0)
    ...
    def record_request_duration(self, processing_time_ms: float, 
    ...
    def record_batch_prediction(self, batch_size: int, processing_time_ms: float):
        ...
        with self.lock:
            ...
            avg_time_per_prediction = processing_time_ms / batch_size if batch_size > 0 else 0
        ...
        with self.lock:
            ...
            return {
                **self.system_metrics,
                ...
                'total_predictions': sum(m.prediction_count for m in self.model_metrics.values()),
                'total_updates': sum(m.update_count for m in self.model_metrics.values()),
    ...
    def _calculate_avg_prediction_time(self) -> float:
        ...
        total_time = sum(m.avg_prediction_time * m.prediction_count for m in self.model_metrics.values())
    ...
    def _calculate_avg_update_time(self) -> float:
        ...
        total_time = sum(m.avg_update_time * m.update_count for m in self.model_metrics.values())
    ...
    def cleanup_old_models(self, max_age_hours: float = 24):
        ...
        with self.lock:
            ...
            for model_id, metrics in self.model_metrics.items():
                last_activity = max(metrics.last_prediction_time, metrics.last_update_time)
...
class AlertManager:
    ...
    def check_alerts(self) -> List[Dict[str, Any]]:
        ...
        for model_id, metrics in self.metrics_collector.get_all_model_metrics().items():
            ...
            # Low accuracy
            if metrics.prediction_count > 100 and metrics.accuracy < self.alert_thresholds['low_accuracy']:
                alert = {
                    'type': 'low_accuracy',
                    'model_id': model_id,
                    'severity': 'critical',
